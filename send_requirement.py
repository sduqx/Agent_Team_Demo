...
if __name__ == "__main__":
    ...
    print("   1️⃣ Backend Agent - 生成 API + 发布完整 API 契约")
    print("   2️⃣ Frontend Agent - 等待 API 契约 → 生成匹配的前端")
    print("   2️⃣ Test Agent - 等待 API 契约 → 生成匹配的测试")
    print("   3️⃣ DevOps Agent - 等待前端+测试完成 → 生成部署配置")
    print()

    requirement = input("📝 请输入你的项目需求（直接回车退出）: ").strip()
    ...
    if requirement:
        ...
        print(f"\n⏳ 请查看各 Agent 窗口的输出以跟踪进度...")
    else:
        print("\n❌ 需求不能为空")
        print("💡 提示: 你也可以直接在 Lead Agent 的窗口输入需求")
