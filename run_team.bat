...
start "Lead Agent" python -m src.lead_agent

timeout /t 2 /nobreak

REM 启动Backend Agent
echo ✓ 正在启动 Backend Agent (后端开发)...
start "Backend Agent" python -m src.backend_agent

timeout /t 1 /nobreak

REM 启动Frontend Agent
echo ✓ 正在启动 Frontend Agent (前端开发)...
start "Frontend Agent" python -m src.frontend_agent

timeout /t 1 /nobreak

REM 启动Test Agent
echo ✓ 正在启动 Test Agent (测试)...
start "Test Agent" python -m src.test_agent

timeout /t 1 /nobreak

REM 启动DevOps Agent
echo ✓ 正在启动 DevOps Agent (部署)...
start "DevOps Agent" python -m src.devops_agent
