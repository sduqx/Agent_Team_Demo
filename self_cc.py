...
def get_shell_executable():
    """
    获取合适的 shell 可执行文件路径。
    Windows: 优先使用 PowerShell Core (pwsh)，其次使用系统默认 (cmd.exe)
    Linux/Mac: 使用系统默认 shell (/bin/bash 或 /bin/sh)
    """
    if IS_WINDOWS:
        # Windows: 尝试找 PowerShell Core
...
def run_bash(command: str) -> str:
    """
    安全执行 shell 命令，支持 Windows 和 Unix-like 系统。
    """
    # 危险命令列表（分别针对 Windows 和 Unix）
    dangerous_windows = [
    ...
    dangerous = dangerous_windows if IS_WINDOWS else dangerous_unix

    if any(d in command.lower() for d in dangerous):
        return "Error: Dangerous command blocked"

    try:
        # 确保 cwd 是字符串格式（Windows 路径兼容性）
...
def run_read(path: str, limit: int = None) -> str:
    ...
    try:
        ...
        # 尝试用其他编码重试
        try:
            lines = safe_path(path).read_text(encoding='gbk').splitlines()
            if limit and limit < len(lines):
                lines = lines[:limit] + [f"... ({len(lines) - limit} more)"]
            return "\n".join(lines)[:50000]
        except Exception as e:
...
class BackgroundManager:
    ...
    def _exec(self, tid: str, command: str, timeout: int):
        try:
            ...
            self.tasks[tid].update({"status": "timeout", "result": "Command timeout"})
        except Exception as e:
...
if __name__ == "__main__":
    ...
    if IS_WINDOWS:
        print("[Windows detected] Using system shell (PowerShell Core if available, else cmd.exe)")
        print("[Tip: Use PowerShell commands like dir, Get-ChildItem, Select-String]")
    elif IS_LINUX:
        print("[Linux detected] Using bash shell")
        print("[Tip: Use Bash commands like ls, grep, find]")
    elif IS_MACOS:
        print("[macOS detected] Using bash shell")
        print("[Tip: Use Bash commands like ls, grep, find]")
    ...
    while True:
        ...
        # 提取 AI 的最后一条回复（从后往前找 assistant 消息）
