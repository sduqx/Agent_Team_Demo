...
Agent Team Demo 是一个基于 LLM 的**多智能体协作开发系统**。系统模拟一个真实的软件开发团队，由多个不同角色的 Agent 组成（Lead + 动态 Worker），通过消息总线进行异步通信，自动完成从需求分析到代码生成的全栈开发流程。
...
- **v5.0：通用 Worker + 动态技术栈** — 不再是固定的 Python Flask 项目，LLM 自主选择任何技术栈
...
- **Phase 1（planning_phase）**：LLM 通过 ReAct 循环分析需求，可调用 `ask_user` 确认模糊点，最终调用 `submit_plan` 提交交换机化的任务计划 JSON
...
### 迭代五：通用化 — 不限技术栈 + 质量保证 (v5.0)

**时间：** 2026年6月4日

**核心思路：** 打破 v4.0"只能做 Python Flask + HTML"的局限，让 LLM 真正做主——自主选技术栈、自主组团队、自主验证产出。

> 📘 详细设计文档 → [`docs/agent-design.md`](./agent-design.md)（v5.0 通用 Worker Agent 章节）
> 📋 快速入门 → [`docs/v5.0-overview.md`](./v5.0-overview.md)
> 📝 更新说明 → [`docs/v5.0-changelog.md`](./v5.0-changelog.md)

**关键变革：**

1. **不限技术栈**：LLM 自主选择 React/Vue/Flutter/Go/Java/Node.js 等任意前后端技术
2. **通用 Worker Agent**：所有 Worker 共用一 `worker_agent.py`，通过 CLI 参数注入角色和技术栈
3. **Lead 自动孵化 Worker**：批准计划后自动为新终端窗口创建 Worker 进程
4. **三层产出验证**：Worker 自验证 → Lead 自动验收 → 人工审查
5. **团队按需组建**：LLM 根据需求决定需要哪些角色，不再永远派 4 个
...
| Agent | 角色 | 产出 | 关键工具 |
|-------|------|------|----------|
| **Lead** | 主编排者 | 需求分析 → 人工审查 → 任务分发 → 监控 + v5.0 技术选型 + Worker 孵化 | `submit_plan`、`ask_user`、`finish_task` |
| **Worker (v4.0)** | 固定 4 个角色 | backend → `api_spec.json` + `app.py`；frontend → `index.html`；test → `tests/`；devops → `README.md` | 全部 7 个通用工具 |
| **Worker (v5.0)** | 通用 1 个文件，动态角色 | 按技术栈产出任意语言/框架的代码 | 全部 8 个通用工具（+ `run_command`） |
...
```
backend（无依赖，最先执行）
   ├── frontend（依赖 backend，等待接口契约）
   ├── test（依赖 backend，等待接口契约）
   └── devops（依赖 frontend + test 都完成）
```

> 注：v5.0 中角色名称和依赖关系完全由 LLM 根据需求动态决定。
...
- **设计模式：** 模板方法、ReAct 循环、工具注册表、单例模式、编排器
- **人工介入：** `ask_user` 阻塞式交互 + Phase 2 人工审查机制
- **v5.0 新增：** Worker 进程孵化、动态 System Prompt、三层产出验证
...
| **v5.0 通用 Worker + 参数化** | 1 个 Worker 文件替代 4 个固定 Agent，技术栈完全自由 |
| **v5.0 三层验证** | Worker 自验证 + Lead 自动验收 + 人工审查，保证产出质量 |
...
| **v4.0** | 三阶段工作流 + 人工审查 + ask_user 交互 | 技术栈固定为 Python Flask + HTML | `b6a86d9` |
| **v5.0** | 不限技术栈 + 通用 Worker + 自动孵化 + 三层验证 | — | — |

整个项目的演进脉络清晰体现了从"能跑起来"→"正确运行"→"优雅运行"→"安全运行"→"通用化"的五段式进化。
...
- **v5.0** 打破技术栈限制，实现**通用多 Agent 协作框架**——LLM 自主选择任何语言、框架、角色组合
